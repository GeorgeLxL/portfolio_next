          ...
          <nav className="hidden md:flex items-center gap-1 ms-auto">
            {navItems.map((item, index) => {
              ...
              return isContact ? (
                ...
                <Link key={item.path} href={item.path} className="px-3 py-2 text-sm transition-colors duration-200 group">
                  <span className={`relative inline-block ${active ? "text-white" : "text-gray-400 group-hover:text-white"}`}>
                    {item.name}
                    <span
                      className={`absolute -bottom-1 left-0 right-0 h-px transition-transform duration-200 origin-left ${
                        active ? "bg-green-400 scale-x-100" : "bg-green-400/50 scale-x-0 group-hover:scale-x-100"
                      }`}
                    />
                  </span>
                </Link>
